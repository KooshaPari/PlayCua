...
The PlayCua-only work that lives outside this repo includes: `native/src/modality/` (5-modality registry: native / sandbox / nvms / wsl / container), `native/src/bin/bare-cua-cli.rs` and `native/src/bin/bare-cua-mcp.rs`, `native/src/mcp_server.rs`, `native/src/socket.rs` (Unix-socket daemon), `native/examples/echo_plugin.rs`, plus `docs/adr/` and `docs/skill-sdk.md`. None of these have been back-ported to bare-cua and none will be — bare-cua is a historical snapshot, not a maintained fork.
