"""laycua.computer - async Python client for laycua-native Rust binary.
...
class Computer:
    """Async context manager that owns the laycua-native subprocess.
    ...
    ----------
    native_path:
        Path or name on PATH of the ``laycua-native`` binary.
    log_level:
    ...
    async def _start(self) -> None:
        ...
        ok = await self.ping()
        if not ok:
            raise RuntimeError("laycua-native did not respond to ping")
    ...
    async def _call(self, method: str, **params: Any) -> Any:
        ...
        async with self._lock:
            ...
            if not raw:
                raise RuntimeError("laycua-native closed stdout unexpectedly")
