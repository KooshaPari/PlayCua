...
PILLARS = [
    ...
    {"id":13,"name":"JUSTFILE","check":lambda p:(p/"Justfile").exists() or (p/"justfile").exists()},
    ...
    {"id":33,"name":"SECURITY_SCANNING","check":lambda p:any((p/".github/workflows"/name).exists() for name in ("codeql.yml","security.yml","trivy.yml","audit.yml","deny.yml")) or any((p/".github/workflows").glob("*secur*.yml")) if (p/".github/workflows").exists() else False},
